...
DEFAULT_OUTPUT_ROOT = Path(
    "/home/pyuser/wkdir/data/GRICS-torch/article_simulated_nonrigid_2D"
)
...
def plot_metric_boxplot(values_list, labels, ylabel, output_path):
    ...
    n_subjects = min(len(v) for v in values_list)

    jitters = rng.uniform(-0.045, 0.045, size=n_subjects)

    for subject_idx in range(n_subjects):
        x_values = positions + jitters[subject_idx]
        y_values = [values[subject_idx] for values in values_list]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot simulated non-rigid reconstruction metrics as SVG bar plots."
    )
