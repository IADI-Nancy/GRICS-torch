...
from utils.affinemotion import motion_model
from utils.createArtifacts import create_mask


def EH(xx, t_n, iterations, masks, sigmas, image_shape, model):
    ...
    for i in range(iterations):
        ...
        for j in range(image_shape[-1]):
            ...
            EHxx_coil[:,:,:,j] = motion_model(added_coil_sensitivity[:,:,:,j], [-t_n[i, 0], -t_n[i, 1], -t_n[i, 2]], model=model)
        EHxx = EHxx + EHxx_coil
...
def E(xx, t_n, iterations, masks, sigmas, image_shape, model):
    ...
    for i in range(iterations):
        ...
        t_xx = motion_model(xx, [t_n[i, 0], t_n[i, 1], t_n[i, 2]], model=model)
...
def EHE(xx, t_n, iterations, masks, sigmas, image_shape, model):
    return EH(E(xx, t_n, iterations, masks, sigmas=sigmas, image_shape=image_shape, model=model), t_n, iterations, masks, sigmas=sigmas, image_shape=image_shape, model=model)